...
| FDA cells — `fda_samd` + `fda_cosmetic` | [phase2.0a](docs/plan/phase2.0a_fda.md) | 🟡 Phase 2 — first non-MFDS authority, **48/61 and building**. eCFR · Federal Register · govinfo connectors live; `cfr_structured` and `usc_text` profiles; English extraction and retrieval; the FD&C Act ingested once and claimed by both cells. The four per-cell trust gates are **not** met — they need signed golden sets and Phase 1 Go — and everything on `fda.gov` is out while its CDN refuses us |
