...
| 2.3 | [phase2.3_product_registries.md](phase2.3_product_registries.md) | Phase 2 · M5–12 | — | ⬜ planned — 510(k) · PMA · UDI · MDR, openFDA's `device` registries, **and the Warning Letters / Import Alerts surfaces re-homed out of 2.0a** — regulatory **fact about products and firms**, not regulation text. Written 2026-08-26 with the sizes measured (~20.7 GB, and `event` alone is 53× the four small registries). **Its own first task is an ADR**: the owner is undecided, and `regulation` · `compliance` · a registry context each have an objection recorded |
